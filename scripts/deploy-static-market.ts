...
import fs from 'fs';
import { StaticMarket } from '../typechain-types';
...
const main = async () => {
    ...
    const staticMarket: StaticMarket = (await StaticMarket.deploy()) as StaticMarket;
    await staticMarket.deployed();

    console.log(`StaticMarket deployed to network ${network} with address: `, staticMarket.address);
};
