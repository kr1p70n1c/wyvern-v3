...
const assertIsRejected = (promise, error_match, message) => {
    ...
    return promise
        ...
        .catch((error) => {
            ...
            if (error_match) {
                if (typeof error_match === 'string') return assert.equal(error_match, error.message, message);
...
const wrap = (inst) => {
    var obj = {
        ...
        atomicMatchWith: (order, sig, call, counterorder, countersig, countercall, metadata, misc) =>
            inst.atomicMatch_(
                [
                    order.registry,
                    order.maker,
        ...
        atomicMatchWith2: (order, sig, call, counterorder, countersig, countercall, metadata, misc, signer) =>
            inst
                .connect(signer)
                .atomicMatch_(
                    [
    ...
    obj.sign = (order, account) => {
        console.log('sign - 1', order, account);
