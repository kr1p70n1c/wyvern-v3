...
contract('WyvernExchange', (accounts) => {
    ...
    const any_erc1155_for_erc20_test = async (options) => {
        ...
        if (buyTokenId) await erc1155.mint(account_a, buyTokenId, erc1155MintAmount);
        ...
        const firstData =
            erc1155c.methods.safeTransferFrom(account_a, account_b, tokenId, sellingNumerator || buyAmount, '0x').encodeABI() +
        ...
        for (var i = 0; i < txCount; ++i) {
            ...
            await exchange.atomicMatchWith(one, sigOne, firstCall, two, sigTwo, secondCall, ZERO_BYTES32, {
                from: sender || account_a
            });
            two.salt++;
        }

        let [account_a_erc20_balance, account_b_erc1155_balance] = await Promise.all([
    ...
    const erc721_for_erc20_test = async (options) => {
        const { tokenId, buyTokenId, sellingPrice, buyingPrice, erc20MintAmount, account_a, account_b, sender } = options;
